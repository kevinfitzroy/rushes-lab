...
- 项目详情顶部"我的角色" chip(直接 / 通过 group / 通过 dept 注明来源)
- folder header"我的有效权限" chip(parent ∪ explicit ∪ sensitive invite 实计算)
- 成员卡片显示"来源"(直接 grant / 通过用户组 X / 通过部门 Y)
- `/my-permissions` 我的权限总览页(所有有权限项目 + 角色 + 临时 grant 倒计时)
