...
| # | 事项 | 影响 |
| --- | --- | --- |
| 1 | 实测离职用户的 `GET /users/:id` 行为(本租户无离职用户) | 验证 `is_resigned=true` 后仍可读 |
| 2 | 实测 `mobile` / `email` 在 IT 后台开放字段权限后的真实返回 | 决定 MS-FB-002 contract 中这两个字段标 `nullable` 还是 `required` |
| 3 | 实测自定义字段 (`custom_attrs`) 配置后的读取格式 | MS-FB-002 是否要把 custom_attrs 透传 |
| 4 | 实测大租户(> 1000 人)的全量同步耗时 + 限频触发阈值 | 决定全量频率(每日 vs 每周) |
| 5 | 实测 `find_by_department(0, fetch_child=true)` 是否能一次性递归到所有员工 | 简化全量同步代码 |
| 6 | `functional_roles` 的"无 LIST + 必须预知 role_id"如何解决 | 看是否能通过事件订阅自建本地 role_id 表,或暂不依赖 |
