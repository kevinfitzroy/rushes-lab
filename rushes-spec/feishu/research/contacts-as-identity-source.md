...
| **角色 (functional_role)** | `POST /functional_roles`(建)、`GET/PUT/DELETE /functional_roles/:role_id`、`POST /functional_roles/:role_id/members/batch_*`、`GET /functional_roles/:role_id/members` | ⚠️ **无 LIST 接口**,应用必须预知 `role_id`(通过持久化或事件订阅)才能查 |
...
**标准等价路径:** `GET /contact/v3/users/find_by_department?department_id=0&user_id_type=open_id&page_size=50`,从根部门(`department_id=0`)拉出所有用户。配合 `has_more` + `page_token` 分页。

辅助路径:
- 先 `GET /departments/0/children?fetch_child=true`(SDK 提到的递归参数)拉完整部门树
- 对每个叶子部门 `find_by_department(d)` 拉成员
...
对账: 每日凌晨一次 find_by_department 全量,与本地缓存 diff,
       发现不一致写告警 + 修正
冷启动 / 重建: 全量同步即可,~100 人级别 < 30s 完成
```
...
| 7 | `open_id` 跨应用域机制深度确认 | 验证机制(union_id 反查 / user_id 检索 / 其他) | 🔴 需要第二个应用,本租户单应用永久 caveat |
...
| `/contact/v3/functional_roles/:role_id/members/batch_delete` | PATCH | 批量删成员 |
...
| 路径 | 描述 | 优缺点 |
| --- | --- | --- |
| (A) **IT 手工告知 bridge** | 在配置文件 / env / DB 维护 `role_id → role_name` 映射,bridge 启动时加载;每加新 role 走运维变更 | 工作量小但易过时;**推荐 v1 走此路** |
| (B) **不依赖 functional_role** | RBAC 维度改用 **部门** + **职级** + **用户组 (group)** 三件套(前两者数据全 + 事件齐;group 数据全但无事件) | 业务表达力可能不够,但 API 完备 |
| (C) **bridge 自维护 role_id 集合** | bridge 监听 user.updated_v3 事件,扫所有 user 的 role 字段(若存在) | (假设 role 字段在 user.updated_v3 事件里有 —— 实测未确认)|

**对 MS-FB-002 / MS-FB-005 契约的影响:**

- MS-FB-002 v1 字段集合**不**含 functional_role 信息(已经一致)
- MS-FB-005 审批人路由若依赖 functional_role,**应采纳 (A) 路径或回到 (B) 路径**;不要假设 bridge 能自动同步全 role 集合

实施时应在 ADR-0001 或 MS-FB-005 起草前显式拍板。本次研究已经做完;**结论:`functional_roles` 不适合作 RBAC 主轴**。
